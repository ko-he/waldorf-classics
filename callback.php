...
if($type == 'follow'){
    ...
    if($get_message == 'password change'){
        ...
        $sql = 'UPDATE line_coin_users SET code=:code WHERE line_id=:id';
        $data = array(
            ':code' => $reset_code,
            ':id' => $userId
        );
        $db->queryPost($sql, $data);
        $message = "Password 変更画面 URL：https://secret-hmyoutuber.ssl-lolipop.jp/line_callback/passwordchenge.php?code=".$reset_code;
    }elseif(strlen($get_message) == 6){
        $sql = 'SELECT * FROM line_coin_users WHERE code=:code';
        ...
        if(empty($row)){
            ...
            $message = "コードの承認が完了しました。\n今後のログインはこちらのURLより可能です。\nhttps://secret-hmyoutuber.ssl-lolipop.jp/line_callback/login";
        }
    }
}
